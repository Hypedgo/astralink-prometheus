...
export default function MissionBrief({ location }: MissionBriefProps) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 10 }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(10px)', border: '1px solid rgba(6,182,212,0.3)', borderRadius: '0.5rem', padding: '1rem', width: '18rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <span style={{ fontFamily: 'monospace', fontSize: '1.125rem', fontWeight: 'bold', color: spaceWeather.loading ? '#9ca3af' : spaceWeather.risk === 'HIGH' ? '#f87171' : spaceWeather.risk === 'MODERATE' ? '#facc15' : '#4ade80' }}>
                                {spaceWeather.loading ? '...' : spaceWeather.kpIndex.toFixed(1)}
                            </span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ color: '#9ca3af', fontSize: '0.875rem' }}>Risk Level</span>
                            <span style={{ padding: '0.25rem 0.5rem', borderRadius: '0.25rem', fontSize: '0.75rem', fontWeight: 'bold', background: spaceWeather.risk === 'HIGH' ? 'rgba(239,68,68,0.2)' : spaceWeather.risk === 'MODERATE' ? 'rgba(234,179,8,0.2)' : 'rgba(34,197,94,0.2)', color: spaceWeather.risk === 'HIGH' ? '#f87171' : spaceWeather.risk === 'MODERATE' ? '#facc15' : '#4ade80' }}>
                                {spaceWeather.risk}
                            </span>
                        </div>
                ...
                <div style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(10px)', border: '1px solid rgba(6,182,212,0.3)', borderRadius: '0.5rem', padding: '1rem', width: '18rem', marginTop: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <span style={{ fontFamily: 'monospace', fontSize: '1.125rem', fontWeight: 'bold', color: weather.loading ? '#9ca3af' : weather.cloudCover < 30 ? '#4ade80' : weather.cloudCover < 60 ? '#facc15' : '#f87171' }}>
                                {weather.loading ? '...' : `${weather.cloudCover}%`}
                            </span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ color: '#9ca3af', fontSize: '0.875rem' }}>Visibility</span>
                            <span style={{ padding: '0.25rem 0.5rem', borderRadius: '0.25rem', fontSize: '0.75rem', fontWeight: 'bold', background: weather.loading ? 'rgba(107,114,128,0.2)' : weather.visibility === 'EXCELLENT' ? 'rgba(34,197,94,0.2)' : weather.visibility === 'GOOD' ? 'rgba(234,179,8,0.2)' : weather.visibility === 'POOR' ? 'rgba(239,68,68,0.2)' : 'rgba(107,114,128,0.2)', color: weather.loading ? '#9ca3af' : weather.visibility === 'EXCELLENT' ? '#4ade80' : weather.visibility === 'GOOD' ? '#facc15' : weather.visibility === 'POOR' ? '#f87171' : '#9ca3af' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <span style={{ color: weather.loading ? '#9ca3af' : weather.cloudCover < 30 ? '#4ade80' : '#facc15' }}>
            ...
            <div style={{ position: 'absolute', right: '1.5rem', top: '72px', pointerEvents: 'auto' }}>
                <div style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(10px)', border: '1px solid rgba(6,182,212,0.3)', borderRadius: '0.5rem', padding: '1rem', width: '20rem' }}>
                    <h2 style={{ color: '#22d3ee', fontSize: '0.875rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', margin: '0 0 0.75rem 0' }}>
                        <span style={{ width: '0.5rem', height: '0.5rem', background: '#facc15', borderRadius: '50%', marginRight: '0.5rem', display: 'inline-block' }}></span>
                        ISS TRACKING
                    </h2>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <h3 style={{ color: '#facc15', fontSize: '0.75rem', fontWeight: 'bold', margin: '0 0 0.5rem 0' }}>NEXT VISIBLE PASSES</h3>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.875rem' }}>
                            {issPasses.loading ? (
                                ...
                                issPasses.passes.slice(0, 3).map((pass, index) => {
                                    ...
                                    return (
                                        <div key={index} style={{ background: index === 0 ? 'rgba(6,182,212,0.1)' : 'rgba(6,182,212,0.05)', borderRadius: '0.25rem', padding: '0.5rem' }}>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem' }}>
                                                ...
                                                <span style={{ color: pass.visibility === 'OPTIMAL' || pass.visibility === 'EXCELLENT' ? '#4ade80' : pass.visibility === 'GOOD' ? '#facc15' : '#9ca3af' }}>
            ...
            <div style={{ position: 'absolute', bottom: '5rem', left: '50%', transform: 'translateX(-50%)', zIndex: 20, pointerEvents: 'auto' }}>
                <button
                    onClick={() => generateBrief(location.latitude, location.longitude, location.name)}
                    disabled={isGenerating}
                    style={{ padding: '0.75rem 1.5rem', background: 'rgba(34,211,238,0.2)', border: '2px solid #22d3ee', borderRadius: '0.5rem', color: '#22d3ee', fontWeight: 'bold', fontSize: '1.125rem', cursor: isGenerating ? 'not-allowed' : 'pointer', opacity: isGenerating ? 0.5 : 1, transition: 'all 0.3s', boxShadow: '0 10px 25px rgba(34,211,238,0.2)' }}
                    onMouseEnter={(e) => { if (!isGenerating) { e.currentTarget.style.background = 'rgba(34,211,238,0.3)'; e.currentTarget.style.boxShadow = '0 10px 25px rgba(34,211,238,0.4)'; } }}
                    onMouseLeave={(e) => { if (!isGenerating) { e.currentTarget.style.background = 'rgba(34,211,238,0.2)'; e.currentTarget.style.boxShadow = '0 10px 25px rgba(34,211,238,0.2)'; } }}
                >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            <span style={{ width: '0.5rem', height: '0.5rem', background: '#4ade80', borderRadius: '50%', marginRight: '0.5rem', display: 'inline-block' }}></span>
                    ...
                    <div style={{ color: '#6b7280', fontSize: '0.75rem' }}>ASTRALINK PROMETHEUS v1.0 | Mission Control Interface</div>
                </div>
            </div>
